    ...
    constructor(props) {
     ...
     componentDidMount(){
      ...
      AsyncStorage.getItem('@user_id').then((userId) => {
        ...
        this.setState({currentQuantity:this.props.route.params.whole_data.quantity});
        if (userId) {
    ...
    onImageClick(item)
    {
      if (this.doubleClick) 
      {
        ...
        imageArr.push(item);
          ...
          this.props.navigation.navigate('ProductDetailImageFullView',{images:imageArr})
      ...
      <StatusBar barStyle="dark-content" backgroundColor={'#fff'} />
        ...
        <TouchableOpacity style={styles.DeleteButtonContainer}
                                    onPress={() => {
                                        ...
                                        <Image source={ require('../images/delete_icon.png')} style={styles.deleteButtonStyle}></Image>
                                    </TouchableOpacity>
        <AppImageSlider
        ...
        {this.state.itemOfProduct.description?(<Text style={{color:'black',marginLeft:30,marginTop:2}}>{this.state.itemOfProduct.description}</Text>):<Text style={{color:'black',marginLeft:30,marginTop:2}}>{"No Description Available"}</Text>}
        {/* {this.state.itemOfProduct.unit?(<Text style={{color:'black',marginLeft:30,marginTop:0}}>Unit {this.state.itemOfProduct.unit}</Text>):null} */}
            </View>
          </Collapsible>          
        ...
        {/* <TouchableOpacity onPress={() => this.props.navigation.goBack()} style={{position: 'absolute',start : 20,top: 30,}}>
            <Image  source={require('../images/back_blck_icon.png')} style={styles.backIcon}/>
             ...
             <Text style={styles.TitleStyle}>{this.state.userName}</Text>  */}
