...
export const forgotPasswordWithOtp = (email_mobile, isMobile, hashCode) => {
    return (dispatch) => getAsyncStorageParameters().then(({ connection_context, odoo_config }) => {
        ...
        return  odoo.rpc_call(endpoint, params).then(response => {
            console.log(">> forgotPasswordWithOtp response", response)
            if(response.success) {
                if(_.get(response, 'data.success') && !_.get(response, 'data.error')) {
                    dispatch({
...
export const submitNewPasswordAfterOtp = (isMobile, email_mobile, newPassword) => {
    return (dispatch) => getAsyncStorageParameters().then(({ connection_context, odoo_config }) => {
        var endpoint = '/web/dataset/call_kw';
        var model = 'res.users';
